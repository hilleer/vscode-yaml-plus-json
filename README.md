...
| `yaml-plus-json.directoryConversion` | Enable the right-click commands that convert all JSON/YAML files in a directory. Set to `false` to hide them from the explorer context menu. **Note:** configured as a flat key, not nested under the `yaml-plus-json` object. | boolean | `true`    | `false`    |
